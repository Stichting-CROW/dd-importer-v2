...
func CountWronglyParkedVehicles(db *sql.DB, date time.Time) {
	log.Print("Counting wrongly parked vehicles...")
	stmt := `
		CREATE TABLE IF NOT EXISTS wrongly_parked_vehicles_output AS
		...
		AND zones.effective_date <= $1 and (zones.retire_date IS NULL OR zones.retire_date > $1)
